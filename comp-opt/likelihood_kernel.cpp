...
static const int PIX_SUM_LANES =  4;
static const int PIX_CHUNKS = (MAX_COUNT_ONES / PIX_SUM_LANES);
...
void compute_likelihood(const int buffer_pixels[N_BUFFER_SIZE][MAX_COUNT_ONES],
    ...
    // arithmetic calculations for final likelihood ratio
    ...
    computeParticles: for (int x = 0; x < N_BUFFER_SIZE; x++) {
        if (x < tileSize){
            ...
            // initialize partial_sum array
            initPartialSums: for (int lane = 0; lane < PIX_SUM_LANES; lane++) {
                #pragma HLS UNROLL
                partial_sum[lane] = 0;
            }

            // accumulate partial_sum in a cyclic manner (every fourth element gets summed into each lane)
...
#pragma HLS ARRAY_PARTITION variable=buffer_pixels cyclic factor=PIX_SUM_LANES dim=2
