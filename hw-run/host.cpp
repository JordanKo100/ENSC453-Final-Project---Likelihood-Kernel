...
constexpr int kRunParticles = 10000;
constexpr int kVerifyParticles = 130;
constexpr int kIszY = 1080;
constexpr int kNfr = 3;
constexpr int kFrameIndex = 1;
constexpr long kMaxSize = 2073600;
...
bool run_case(OpenClSession& session,
    ...
    if (!verify) {
        std::cout << "[" << label << "] likelihood checksum = " << std::setprecision(15)
                  << checksum_wide(likelihood_wide, static_cast<std::size_t>(particle_count)) << "\n";
        return true;
    }
    ...
    std::cout << "[" << label << "] checksum = " << std::setprecision(15)
              << checksum_wide(likelihood_wide, static_cast<std::size_t>(particle_count))
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        if (do_verify) {
            bool pass = true;
            pass &= run_case(session, "verify_interior", kVerifyParticles, false, true);
            pass &= run_case(session, "verify_boundary", kVerifyParticles, true, true);
            std::cout << (pass ? "TEST PASSED\n" : "TEST FAILED\n");
            return pass ? EXIT_SUCCESS : EXIT_FAILURE;
        }

        if (!run_case(session, "hardware_run", kRunParticles, false, false)) {
            return EXIT_FAILURE;
        }
        return EXIT_SUCCESS;
