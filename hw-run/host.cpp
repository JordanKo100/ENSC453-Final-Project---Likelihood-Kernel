...
void compute_reference(int Nparticles, int countOnes, int IszY, int Nfr, int k, long max_size,
    ...
    for (int x = 0; x < Nparticles; x++) {
        int px = shared_roundDouble(arrayX[x]);
        int py = shared_roundDouble(arrayY[x]);
        double sum = 0.0;
        for (int y = 0; y < countOnes; y++) {
            int offY = shared_roundDouble(objxy[y * 2]);
            int offX = shared_roundDouble(objxy[y * 2 + 1]);
            int indX = px + offX;
            int indY = py + offY;
            ...
            int a = pix - (int)PIXEL_A_OFFSET;
            int b = pix - (int)PIXEL_B_OFFSET;
            sum += ((double)(a * a) - (double)(b * b)) / PIXEL_DENOM;
        }
...
int main(int argc, char** argv) {
    ...
    // Execute Realistic Production Stress Tests
    pass &= run_case(context, q, kernel, "production_10k_interior", 10000, false, TB_MAX_SIZE, objxy.data(), countOnes, I.data());
    pass &= run_case(context, q, kernel, "production_10k_boundary", 10000, true, TB_MAX_SIZE, objxy.data(), countOnes, I.data());
